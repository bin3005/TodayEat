package view.adminview.leftpanel;
...
import view.adminview.rightpanel.AddRestaurantPanel;
...
public class LeftPanel1 extends JPanel implements leftpanel { // 왼쪽 패널
	private RestaurantFrame mf;

	private int prePage;

	public LeftPanel1(RestaurantFrame mf, int prePage) {// 생성자
		//////////
		/// 기본적인 구조 설계
		...
		for (int k = 0; k < (( restList.size() - 1) / 10) + 1; k++) {
			...
			//////////////////////////////
			// 식당 추가
			JButton resPlus = new JButton("식당 추가");
			resPlus.setBounds(200, 400, 91, 23);
			resPlus.setBackground(Color.black);
			resPlus.setForeground(Color.white);
			resPlus.setBorder(new LineBorder(Color.LIGHT_GRAY, 2));
			//// 식당 추가 이벤트
			resPlus.addActionListener(new ActionListener() {

				@Override
				public void actionPerformed(ActionEvent e) {
					AddRestaurantPanel rightPanel = new AddRestaurantPanel(mf, getThis());
					changePanel(rightPanel);
				}
			});

			resButton.add(resPlus);
			/////////////////////
			/// 식당삭제
			JButton resDelete = new JButton("식당 삭제");
			resDelete.setBounds(300, 400, 91, 23);
			resDelete.setBackground(Color.black);
			resDelete.setForeground(Color.white);
			resDelete.setBorder(new LineBorder(Color.LIGHT_GRAY, 2));
			//// 식당 삭제 이벤트
			resDelete.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					changePanel(rightpanel);
					String resName = JOptionPane.showInputDialog(mf, "삭제할 식당이름을 입력하세요.", "식당 삭제",
							JOptionPane.QUESTION_MESSAGE);
					if (resName != null) {

						int res = mf.getRc().deleteRestaurant(resName);
						if (res > 0) {
							JOptionPane.showMessageDialog(null, "식당 삭제가 정상적으로 완료되었습니다!!", "성공했습니다",
									JOptionPane.INFORMATION_MESSAGE);
							System.out.println(resName + "식당 Delete 성공!");
							
							changePanel(new LeftPanel1(mf,0));
						} else {
							JOptionPane.showMessageDialog(null, "동일한 이름을 가진 식당이 존재하지 않습니다!!!", "오류",
									JOptionPane.ERROR_MESSAGE);
							System.out.println("[ERROR]: 동일한 이름을 가진 식당이 존재하지않음." + resName + "식당 Delete 실패!");
						}
					}
				}
			});

			resButton.add(resDelete);
			...
			nextPage.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					if ((mf.getRc().selectAllRestaurant().size() - 1) / 10 > getThis().prePage) {
						getThis().remove(panelList.get(getThis().prePage));
						getThis().prePage += 1;
						getThis().add(panelList.get(getThis().prePage));
						mf.repaint();
